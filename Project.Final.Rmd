...
# Introduction: 
  ...
  Our objectives include evaluating how depth affects each flux and how each flux affects mass flux. Our hypothesis are as follows. 
H1: 
H0: Carbon flux does not change between 150 and 200 meter.
HA: Carbon flux is lower at 200 meter than at 150 meter.

H2: 
H0: Carbon flux does not change between 200 and 300 meter.
HA: Carbon flux is lower at 300 meter than at 200 meter.

H3: 
H0: Nitrogen flux does not change between 150 and 200 meter.
HA: Nitrogen flux is lower at 200 meter than at 150 meter.

H4:
H0: Nitrogen flux does not change between 200 and 300 meter.
HA: Nitrogen flux is lower at 300 meter than at 200 meter.
H5: 
H0: Phosphorus flux does not change between 150 and 200 meter.
HA: Phosphorus flux is lower at 200 meter than at 150 meter.

H6:
H0: Phosphorus flux does not change between 200 and 300 meter.
HA: Phosphorus flux is lower at 300 meter than at 200 meter.

H7: 
H0: Mass flux does not change between 150 and 200 meter.
HA: Mass flux is lower at 200 meter than at 150 meter.

H8:
H0: Mass flux does not change between 200 and 300 meter.
HA: Mass flux is lower at 300 meter than at 200 meter.
...
Our Data was subset from the original data set to exclude all blank columns and include just the average flux values from each of the samples taken at a specific cruise, location, and depth. Each Flux was then subset into their own data frame and then each depth was subset into their own dataframe. These data frames were then used to take statistical inference in the form of number of observations, means and standard error from each flux at each depth. The means were then used to visually access the difference in each depth of each flux in the form of barplots. To check for distribution normality and outliers, histograms and boxplots for each flux at each depth were created. Log transformations for all flux values were done to increase normality and eventually linearity in regression models. To test for significant difference in flux values as depth increases, paired sample t-test were performed. Two paired sample t-test was performed for each flux, comparing flux values at 150 meters to 200 meters and 200 meters to 300 meters. All 400 meter values were excluded from hypothesis testing due to low observational units and non-Gaussian distributions, even after log transformations. A paired sample t-test was chosen because samples were collected on the same cruise and at the same location, but at differnt depths. So for each flux value at one depth, there was a flux value collected at a different depth at the same location. Data frames for each paired sample t-test were merged based on cruise, year, and location to have the same number of values. The dbar (ybar_1 - ybar_2) was created for  the difference between each pair,and histograms and boxplots were made to look at normality and outliers. Natural log transformations for dbar was needed to increase normality. Paired sample-test with a 95% confidence interval were preformed for each depth comparison. Our null hypothesis is that the avergage flux at each depth is the same, our alternative hypothesis being that shallower depths would have a greater flux value then deeper depth. We then used simple linear regression to determine how well different nutrient flux predict mass flux. When inspecting a plot of the entire data set, collinearity can be seen between carbon and nitrogren. These carbon and nitrogen flux were then combined and plotted as a predictor of mass flux. Simple linear regression of depth as predictors of all the fluxes were preformed. Multiple regression analysis was preformed using both depth and cabron,nitrogen flux combined as predictors for mass flux. We then did two anova tests to compare the fit our simple linear regression models (small) of carbon,nitrogen flux combined predicting mass flux, and depth predicting mass flux, against our multple regression model (large) that used both flux and depth to predict mass flux. Ancova model was then created to look at how all variables predict mass flux: carbon, nitrogen, phosphorous, depth, and month and year as blocking factors to account for interanual and anual variation.  

# Implementation:

*See Appendix i for all code*
...
*See Appendix ii for distributional hitrograms, log tranformation histograms, and boxpplots*
...
Boxplot means for carbon flux overlap, but box sizes for 150, 200, 300, and 400 seem to be similar in box size as well as whisker length which means that they could be good predictors of each other. However, the concern is the overlap suggesting that they break homoscedasticity and are not good predictors of one another.
...
Boxplot means for nitroen flux seem to overlap a bit for 150, 200, 300, and 400. The 400 depth has a lot of overlap and a very different shape and whiskers. All of these observations suggest that the model breaks homosedasticity and that the depths are not good good predictors of one another.
...
Boxplot means for phosphorous flux seem to overlap a for 150, 200, and 300. Even though they are similar in box and whisker length, the overlap suggests that the model breaks homosedasticity and that the depths are not good good predictors of one another.
...
Boxplot means for mass flux seem to have some overlap. They are similar in box size except for 400, but the concern is the overlap, making them not good predictors of one another and potentially breaking homosedasticity.

<br>

Conclusively, flux at one depth is not good predictors of the sae flux at another depth and will no be used in our analysis. 
...
All other depth flux distributions increased normality with log transformation.
...
# Nitrogen
par(mfrow=c(1,3))
hist(N.depth.200.300$D, breaks = "FD", main = "Fig 18 N.depth.200.300$D")
hist(log(N.depth.200.300$D), breaks = "FD", main ="Fig 19 N.depth.200.300$D_bar") #try lo transform 
boxplot(log(N.depth.200.300$D), main = "Fig 20 N.depth.200.300$D") #lots of outliers 
```
...
# Phosphorous
par(mfrow=c(1,3))
hist(P.depth.150.200$D, breaks = "FD", main ="Fig 21 P.depth.150.200$D_bar")
hist(log(P.depth.150.200$D), breaks = "FD", main ="Fig 22 P.depth.150.200$D_bar") #try lo transform 
boxplot(log(P.depth.150.200$D), main ="Fig 23 P.depth.150.200$D_bar") #lots of outliers 
```
...
# Phosporous
par(mfrow=c(1,3))
hist(P.depth.200.300$D, breaks = "FD", main ="Fig 24 P.depth.200.300$D_bar")
hist(log(P.depth.200.300$D), breaks = "FD", main ="Fig 25 P.depth.200.300$D_bar") #try lo transform 
boxplot(log(P.depth.200.300$D), main ="Fig 26 P.depth.200.300$D_bar") #lots of outliers 
```
...
# Mass 
par(mfrow=c(1,3))
hist(M.depth.150.200$D, breaks = "FD", main ="Fig 27 M.depth.150.200$D_bar")
hist(log(M.depth.150.200$D), breaks = "FD", main ="Fig 28 M.depth.150.200$D_bar") #try lo transform 
boxplot(log(M.depth.150.200$D), main ="Fig 29 M.depth.150.200$D_bar") #lots of outliers 
```
...
# Mass
par(mfrow=c(1,3))
hist(M.depth.200.300$D, breaks = "FD", main ="Fig 30 M.depth.200.300$D_bar")
hist(log(M.depth.200.300$D), breaks = "FD", main ="Fig 31 M.depth.200.300$D_bar") #try lo transform 
boxplot(log(M.depth.200.300$D), main ="Fig 32 M.depth.200.300$D_bar") #lots of outliers 
```
...
par(mfrow=c(1,3))
plot(log(Data.noNA$C_avg),log(Data.noNA$M_avg), main ="Fig 33 M v. C") #good linearity, but clumping
lmfit_C.M <- lm(log(M_avg) ~ log(C_avg), data = Data.noNA)
summary(lmfit_C.M)
abline(lmfit_C.M)

plot(log(Data.noNA$N_avg),log(Data.noNA$M_avg), main ="Fig 34 M v. N") # alright linearity, still clumping 
lmfit_N.M <- lm(log(M_avg) ~ log(N_avg), data = Data.noNA)
summary(lmfit_N.M)
abline(lmfit_N.M)

plot(log(Data.noNA$P_avg),log(Data.noNA$M_avg), main ="Fig 35 M v. P") # worse, still clumping 
...
*See Appendix ii for all EDA output*
...
hist(resids, breaks=20, main = "Fig 36")
qqnorm(resids)
qqline(resids) # add straight line from true normal

plot(resids, main="Fig 37 resid vs i") 
abline(h=0) # mean of epsilon_hat

plot(x=fit, y=resids, main="Fig 38 resid vs y_hat")
abline(h=0)
```
...
hist(resids, breaks=20, main = "Fig 39")
qqnorm(resids)
qqline(resids) # add straight line from true normal

plot(resids, main="Fig 40 resid vs i") 
abline(h=0) # mean of epsilon_hat

plot(x=fit, y=resids, main="Fig 41 resid vs y_hat")
abline(h=0)
...
hist(resids, breaks=20, main = "Fig 42")
qqnorm(resids)
qqline(resids) # add straight line from true normal

plot(resids, main="Fig 43 resid vs i") 
abline(h=0) # mean of epsilon_hat

plot(x=fit, y=resids, main="Fig 44 resid vs y_hat")
abline(h=0)
```
...
lmfit.flux <- lm(log(M_avg) ~ log(flux),
         data=Data.noNA)
summary(lmfit.flux)
plot(log(Data.noNA$flux), log(Data.noNA$M_avg), main ="Fig 45")
abline(lmfit.flux)
```
```{r,echo=FALSE,include=FALSE}
...
hist(resids, breaks=20, main ="Fig 46")
qqnorm(resids)
qqline(resids) # add straight line from true normal

plot(resids, main="Fig 47 resid vs i") 
abline(h=0) # mean of epsilon_hat

plot(x=fit, y=resids, main="Fig 48 resid vs y_hat")
abline(h=0)

```

*We see collineary between Nitrogen and Carbon flux predictors, so they were combined into a singal flux categorie to predict mass flux*
...
plot(Data.noNA$dep, log(Data.noNA$C_avg), main ="Fig 49")
lmfit_Dep.C <- lm(log(C_avg) ~ dep, data = Data.noNA)
summary(lmfit_Dep.C)
abline(lmfit_Dep.C)

plot(Data.noNA$dep, log(Data.noNA$N_avg), main= "Fig 50")
lmfit_Dep.N <- lm(log(N_avg) ~ dep, data = Data.noNA)
summary(lmfit_Dep.N)
abline(lmfit_Dep.N)

plot(Data.noNA$dep, log(Data.noNA$P_avg),  main= "Fig 51")
lmfit_Dep.P <- lm(log(P_avg) ~ dep, data = Data.noNA)
summary(lmfit_Dep.P)
abline(lmfit_Dep.P)

plot(Data.noNA$dep, log(Data.noNA$M_avg),  main= "Fig 52")
...
hist(resids, breaks=20,  main= "Fig 53")
qqnorm(resids)
qqline(resids) # add straight line from true normal

plot(resids, main="Fig 54 resid vs i") 
abline(h=0) # mean of epsilon_hat

plot(x=fit, y=resids, main="Fig 55 resid vs y_hat")
abline(h=0)
```

Multiple regression plot shows lower $R^2$ then simple linear regression plot with just flux predictors.We see strong signals in our intercept and flux, but weak signal in depth in summary output. 

<br>

#### which one is better?

Comparing simple linear regression of carbon,nitrogen combined flux to multple regression of flux + depth.
...
Comapring simple linear regression of depth to multple regression of flux + depth. 
...
Depth is our categorical predictor, different nutrition fluxes are numerical predictors. 
...
pairs.panels(data.frame(as.factor(dep),log(C_avg),log(N_avg),log(P_avg),log(M_avg)), main = "Fig 56")

xyplot(log(M_avg) ~ log(C_avg) | as.factor(dep), 
                  main="Fig 57: Activity level-specific scatterplots"
           )

    xyplot(log(M_avg) ~ log(C_avg), groups=as.factor(dep), 
                  auto.key=TRUE,
                  main="Fig 58: Scatterplot with color=group level"
           )
    ...
    xyplot(log(M_avg) ~ log(C_avg), groups=as.factor(dep), 
                  ...
                  auto.key=TRUE,
                  main="Fig 59: Three standalone depth level-specific `lm()` fits"
           )
```

*After log transforming data, C, N, P, and M distributions look much more noraml. Linearlity is looking better, especially for C and N. Activity level specific scatterplots highlight the interaction with C,N, and P flux with M flux through depths and have somewhat of a linear reationship. Scatterplot combined, shows overlap between our predictors*
...
Flux consistently decreases with depth in all parameters, carbon, nitrogen, phosphorus, and mass (Fig, 1-4).

In order to perform t-tests with our data we needed to first check for normality and outliers in our data (Fig 15- 32). All fluxes went through log transformations.

After running 95 percent confidence intervals for each flux compared to depth, we reject the null for H1. as carbon flux is lower at 200 meters than at 150 meters (t = 14.668, df = 302, p-value < 2.2e-16). We reject the null for H2 as carbon flux is lower at 300 meters than at 200 meters (t = 13.324, df = 299, p-value < 2.2e-16). We reject the null for H3 as nitrogen flux is lower at 200 meters than at 150 meters (t = 14.918, df = 300, p-value < 2.2e-16). We reject the null for H4 as nitrogen flux is lower at 300 meters than at 200 meters (t = 15.938, df = 297, p-value < 2.2e-16). We reject the null for H5 as phosphorus flux is lower at 200 meters than at 150 meters (t = 10.941, df = 105, p-value < 2.2e-16). We reject the null for H6 as phosphorus flux is lower at 300 meters than at 200 meters (t = 12.446, df = 104, p-value < 2.2e-16). We reject the null for H7 as mass flux is lower at 200 meters than at 150 meters (t = 11.049, df = 303, p-value < 2.2e-16). We reject the null for H8 as mass flux is lower at 300 meter then at 200 meter (t = 12.054, df = 300, p-value < 2.2e-16). 

In order to see how well P, N, and C were able to predict mass flux, simple linear regression was performed with log transformed data (Fig 33, 34, 35). C-flux was the best predictor of mass flux, but still had a little clumping (Fig 33, r2=0.7405). N-flux was the next best predictor (Fig 34, r2 = 0.6446). P-flux was the worst predictor of mass flux (Fig 35, r2=0.3592).

Even though carbon-flux seemed to be a good predictor of mass flux in the simple linear regression model, after running EDA we see that the model is not very normal, poor linearity, and both residual are not very evenly scattered and there is clumping around the line, showing that they are not heteroscedasticity. All these models suggest that trifecta is broken (Fig 36, 37, 38). We see this same trend as we look at N-flux as a predictor (Fig 39, 40, 41). As well as P-flux (Fig 42, 43, 44).

We next wanted to see if collinearity between the different fluxes that are predicting mass flux and potentially combine the fluxes, so we ran a ANOVA regression model and best fit line model. Collinearity was observed between Nitrogen and Carbon flux predictors, so they were combined into a single flux categorie to predict mass flux (Fig 45-48).

Next, we ran a regression model to see how depth affects flux, and we see that as depth decreases so does flux, so depth can be a predictor of flux (Fig 49-52).

We ran a second ANOVA model with an extra parameter to see if this would improve the model fit. The extra parameter did improve the mode and made it more normal and linear, but was still very clustered for residuals (Fig 53-55).

In comparing our two Anova models, we see that we see that there is no significant difference in the first anova model, as our p-value is too large to assume that losing df due to an extra parameter is worthwhile. However, our p-value is significantly small in our second anova model indicating that losing df due to an extra parameter is worthwhile, and improves the fit.

We then ran an ANCOVA model. After log transforming data, C, N, P, and M distributions look much more normal and linearity is looking better, especially for C and N. Activity level specific scatterplots highlight the interaction with C and M through depths and have somewhat of a linear relationship. However, the three standalone depth level-specific lm fits are not parallel to each other, suggesting that they may not be good predictors of one another (Fig 56-59).

# Discussion:

Through all our model tests. We see that flux consistently decreases with depth in each, N, P, C, and M. Depth can be used as a predictor of flux. We also see that C-flux seems to be the best predictor of mass flux. This could be potentially due to the additional carbon flux promoted by biological organisms such as copepods that excrete fecal pellets that are able to sink this carbon to depth (7). In our second anova model, we see that the best way to predict mass flux is by using all of the flux averages as well as depth for parameters. We expected to see these trends as previous literature had suggested that flux decrease with depth (4).

# Limitations:
We had limitations in our data from a lot of N/As that prevented us on running full analyses. If this data had been available, we could have seen much stronger trends in our data. We did see numerous outliers in our data, but proceeded with hypothesis testing anyways, could have led to unreliable results.  
...
7. H. van der Jagt, I. Wiedmann, N. Hildebrandt, B. Niehoff, M. H. Iversen, Aggregate Feeding by the Copepods Calanus and Pseudocalanus Controls Carbon Flux Attenuation in the Arctic Shelf Sea During the Productive Period. Frontiers in Marine Science 7, 16 (2020).
...
### Distribution of phosporous flux
...
### EDA for nitrogen and carbon flux combined
