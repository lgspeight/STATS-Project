...
*boxplot means do not overlap, and box sizes for 150, 200, 300, and 400 seem to be similar in box size as well as whisker length which means that they could be good predictors of each other and do not break homoscedasticity. However, the 400 depth has some overlap which could break homosedasticity*
...
*boxplot means seem to have some overlap, but not too much and they are similar in box size and whisker length*
...
*boxplot means seem to have some overlap. They are similar in box size except for 400, but the concern is the overlap, making them not good predictors of one another and potentially breaking homosedasticity*
...
In order to perform t-tests with our data we needed to first check for normality and outliers in our data. All fluxes went through log transformations. C-flux boxplot boxplot means do not overlap, and box sizes for 150, 200, 300, and 400 seem to be similar in box size as well as whisker length which means that they could be good predictors of each other and do not break homoscedasticity. However, the 400 depth has some overlap which could break homosedasticity (Fig 5). N-flux boxplot means do not too seem to overlab too much for 150, 200, and 300 but ther is a bit of overlap in 300 and 400. 150, 200, and 300 seem to be similar in box size as well as whisker length which means that they could be good predictors of each other and do not break homoscedasticity. However, the 400 depth has a lot of overlap and a very different shape and whiskers which could break homosedasticity (Fig 6). For P-flux, boxplot means seem to have some overlap, but not too much and they are similar in box size and whisker length (Fig 7). For M-flux, boxplot means seem to have some overlap. They are similar in box size except for 400, but the concern is the overlap, making them not good predictors of one another and potentially breaking homoscedasticity (Fig 8). 
...
In order to see how well P, N, and C were able to predict mass flux, simple linear regression was performed with log transformed data. C-flux was the best predictor of mass flux, but still had a little clumping (Fig 58, r2=0.7405). N-flux was the next best predictor (Fig 59, r2 = 0.6446). P-flux was the worst predictor of mass flux (Fig 60, r2=0.3592).

Even though carbon-flux seemed to be a good predictor of mass flux in the simple linear regression model, after running EDA we see that the model is not very normal, poor linearity, and both residual are not very evenly scattered and there is clumping around the line, showing that they are not heteroscedasticity. All these models suggest that trifecta is broken (Fig 61 and 62 with associated distribution and linear fit model). We see this same trend as we look at N-flux as a predictor (Fig 63 and 64 with associated distribution and linear fit model). As well as P-flux (Fig 65 and 66 with associated distribution and linear fit model).
We next wanted to see if collinearity between the different fluxes that are predicting mass flux and potentially combine the fluxes, so we ran a ANOVA regression model and best fit line model. Collinearity was observed between Nitrogen and Carbon flux predictors, so they were combined into a single flux categorie to predict mass flux (Fig 67-68).

Next, we ran a regression model to see how depth affects flux, and we see that as depth decreases so does flux, so depth can be a predictor of flux (Fig 70-73).

We ran a second ANOVA model with an extra parameter to see if this would improve the model fit. The extra parameter did improve the mode and made it more normal and linear, but was still very clustered for residuals (Fig 74-76).
In comparing our two Anova models, we see that we see that there is no significant difference in the first anova model, as our p-value is too large to assume that losing df due to an extra parameter is worthwhile (Fig 67-69). However, our p-value is significantly small in our second anova model indicating that losing df due to an extra parameter is worthwhile, and improves the fit (Fig 74-76).
